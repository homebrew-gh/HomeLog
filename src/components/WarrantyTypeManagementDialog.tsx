...
export function WarrantyTypeManagementDialog({ isOpen, onClose }: WarrantyTypeManagementDialogProps) {
  const { 
    customWarrantyTypes, 
    visibleDefaultTypes,
  ...
  const handleAddType = () => {
    ...
    if (!trimmed) {
      toast({
        ...
        description: 'Please enter a warranty type name.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (allWarrantyTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      toast({
        title: 'Type already exists',
        description: 'This warranty type is already in your list.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (hiddenDefaultWarrantyTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      const matchingType = hiddenDefaultWarrantyTypes.find(t => t.toLowerCase() === trimmed.toLowerCase());
      if (matchingType) {
        restoreDefaultWarrantyType(matchingType);
        setNewType('');
        toast({
          title: 'Type restored',
          description: `"${matchingType}" has been restored to your warranty types.`,
        });
        return;
      }
    }

    addCustomWarrantyType(trimmed);
    setNewType('');
    toast({
      title: 'Type added',
      description: `"${trimmed}" has been added to your warranty types.`,
    });
  };
  ...
  const handleRemoveType = (type: string) => {
    ...
    toast({
      title: 'Type removed',
      description: `"${type}" has been removed from your warranty types.`,
    });
  };

  const handleRestoreType = (type: string) => {
    restoreDefaultWarrantyType(type);
    toast({
      title: 'Type restored',
      description: `"${type}" has been restored to your warranty types.`,
    });
  };
  ...
  const sortedCustomTypes = [...customWarrantyTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  const sortedHiddenTypes = [...hiddenDefaultWarrantyTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[90dvh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Manage Warranty Types</DialogTitle>
        </DialogHeader>
