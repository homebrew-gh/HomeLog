...
interface CompanyTypeManagementDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function CompanyTypeManagementDialog({ isOpen, onClose }: CompanyTypeManagementDialogProps) {
  const {
    customCompanyTypes,
    visibleDefaultTypes,
  ...
  const [typeWithCompanies, setTypeWithCompanies] = useState<{ name: string; count: number } | null>(null);
  ...
  const handleAddType = () => {
    ...
    if (!trimmed) {
      toast({
        ...
        description: 'Please enter a company or service type name.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (allCompanyTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      toast({
        title: 'Type already exists',
        description: 'This company/service type is already in your list.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (hiddenDefaultCompanyTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      const matchingType = hiddenDefaultCompanyTypes.find(t => t.toLowerCase() === trimmed.toLowerCase());
      if (matchingType) {
        restoreDefaultCompanyType(matchingType);
        setNewType('');
        toast({
          title: 'Type restored',
          description: `"${matchingType}" has been restored to your company types.`,
        });
        return;
      }
    }

    addCustomCompanyType(trimmed);
    setNewType('');
    toast({
      title: 'Type added',
      description: `"${trimmed}" has been added to your company types.`,
    });
  };

  const handleRemoveType = (type: string) => {
    const companiesWithType = companies.filter(c => c.serviceType === type);

    if (companiesWithType.length > 0) {
      setTypeWithCompanies({ name: type, count: companiesWithType.length });
      return;
    }
    ...
    toast({
      title: 'Type removed',
      description: `"${type}" has been removed from your company types.`,
    });
  };

  const handleRestoreType = (type: string) => {
    restoreDefaultCompanyType(type);
    toast({
      title: 'Type restored',
      description: `"${type}" has been restored to your company types.`,
    });
  };
  ...
  const sortedCustomTypes = [...customCompanyTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  const sortedHiddenTypes = [...hiddenDefaultCompanyTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[90dvh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Manage Company / Service Types</DialogTitle>
        </DialogHeader>
      ...
      {/* Warning dialog for types with companies */}
      <AlertDialog open={!!typeWithCompanies} onOpenChange={() => setTypeWithCompanies(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                The type &quot;{typeWithCompanies?.name}&quot; is still in use by {typeWithCompanies?.count} compan{typeWithCompanies?.count !== 1 ? 'ies' : 'y'}.
              </p>
              <p>
                To delete this type, please first delete or change the type of all companies using this type.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => setTypeWithCompanies(null)}>
              Understood
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
