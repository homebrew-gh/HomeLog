...
interface VehicleTypeManagementDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function VehicleTypeManagementDialog({ isOpen, onClose }: VehicleTypeManagementDialogProps) {
  const { 
    customVehicleTypes, 
    visibleDefaultTypes,
  ...
  const [typeWithVehicles, setTypeWithVehicles] = useState<{ name: string; count: number } | null>(null);
  ...
  const handleAddType = () => {
    ...
    if (allVehicleTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      toast({
        title: 'Type already exists',
        description: 'This vehicle type is already in your list.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (hiddenDefaultVehicleTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      const matchingType = hiddenDefaultVehicleTypes.find(t => t.toLowerCase() === trimmed.toLowerCase());
      if (matchingType) {
        restoreDefaultVehicleType(matchingType);
        setNewType('');
        toast({
          title: 'Type restored',
          description: `"${matchingType}" has been restored to your vehicle types.`,
        });
        return;
      }
    }

    addCustomVehicleType(trimmed);
    setNewType('');
    toast({
      title: 'Type added',
      description: `"${trimmed}" has been added to your vehicle types.`,
    });
  };

  const handleRemoveType = (type: string) => {
    // Check if any vehicles are assigned to this type
    const vehiclesWithType = vehicles.filter(v => v.vehicleType === type);

    if (vehiclesWithType.length > 0) {
      // Show warning dialog
      setTypeWithVehicles({ name: type, count: vehiclesWithType.length });
      return;
    }
    ...
    toast({
      title: 'Type removed',
      description: `"${type}" has been removed from your vehicle types.`,
    });
  };

  const handleRestoreType = (type: string) => {
    restoreDefaultVehicleType(type);
    toast({
      title: 'Type restored',
      description: `"${type}" has been restored to your vehicle types.`,
    });
  };
  ...
  const sortedCustomTypes = [...customVehicleTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  const sortedHiddenTypes = [...hiddenDefaultVehicleTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      ...
      {/* Warning dialog for types with vehicles */}
      <AlertDialog open={!!typeWithVehicles} onOpenChange={() => setTypeWithVehicles(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                The type "{typeWithVehicles?.name}" has {typeWithVehicles?.count} vehicle{typeWithVehicles?.count !== 1 ? 's' : ''} assigned to it.
              </p>
              <p>
                To delete this type, please first delete or change the type of all vehicles with this type.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => setTypeWithVehicles(null)}>
              Understood
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
