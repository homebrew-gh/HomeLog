...
export function SubscriptionTypeManagementDialog({ isOpen, onClose }: SubscriptionTypeManagementDialogProps) {
  const { 
  ...
  const handleAddType = () => {
    ...
    if (!trimmed) {
      toast({
        ...
        description: 'Please enter a subscription type name.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (allSubscriptionTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      toast({
        title: 'Type already exists',
        description: 'This subscription type is already in your list.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (hiddenDefaultSubscriptionTypes.some(t => t.toLowerCase() === trimmed.toLowerCase())) {
      const matchingType = hiddenDefaultSubscriptionTypes.find(t => t.toLowerCase() === trimmed.toLowerCase());
      if (matchingType) {
        restoreDefaultSubscriptionType(matchingType);
        setNewType('');
        toast({
          title: 'Type restored',
          description: `"${matchingType}" has been restored to your subscription types.`,
        });
        return;
      }
    }

    addCustomSubscriptionType(trimmed);
    setNewType('');
    toast({
      title: 'Type added',
      description: `"${trimmed}" has been added to your subscription types.`,
    });
  };
  ...
  const handleRemoveType = (type: string) => {
    ...
    toast({
      title: 'Type removed',
      description: `"${type}" has been removed from your subscription types.`,
    });
  };

  const handleRestoreType = (type: string) => {
    restoreDefaultSubscriptionType(type);
    toast({
      title: 'Type restored',
      description: `"${type}" has been restored to your subscription types.`,
    });
  };
  ...
  const sortedCustomTypes = [...customSubscriptionTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
  const sortedHiddenTypes = [...hiddenDefaultSubscriptionTypes].sort((a, b) =>
    a.toLowerCase().localeCompare(b.toLowerCase())
  );
